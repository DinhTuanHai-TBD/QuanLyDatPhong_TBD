...
// 3. Rewrite submitBooking
const submitBookingStart = file.indexOf('const submitBooking = (values: SubmitFormValues) => {');
const submitBookingEndStr = '  const handleBookingSuccess =';
const doSubmitBookingEnd = file.indexOf(submitBookingEndStr, submitBookingStart);
...
const newSubmitLogic = `  const submitBooking = (values: SubmitFormValues) => {
    ...
    createMutation.mutate({
      ...
      onSuccess: () => {
        handleBookingSuccess(newBooking as any, userEmail);
      },
      onError: (err) => {
        console.error(err);
