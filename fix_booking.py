...
# 1. Imports
content = re.sub(r"Checkbox, Modal \} from 'antd'", r"Checkbox, Modal, Collapse } from 'antd'", content)

# 2. Remove states
content = re.sub(r"  const \[confirmModalVisible.*?\n", "", content)
content = re.sub(r"  const \[pendingSubmitValues.*?\n", "", content)
content = re.sub(r"  const \[agreedInModal.*?\n", "", content)

# 3. Replace submitBooking and doSubmitBooking
submit_pattern = r"  const submitBooking = \(values: SubmitFormValues\) => \{.*?\n  \}"
content = re.sub(submit_pattern, r"""  const submitBooking = (values: SubmitFormValues) => {
...
# Delete doSubmitBooking
content = re.sub(r"  const doSubmitBooking = \(\) => \{.*?\n  \}\n", "", content, flags=re.DOTALL)

# 4. Replace form layout
form_pattern = r"        <Col xs=\{24\} lg=\{8\}>.*?        <Col xs=\{24\} lg=\{16\}>"
new_layout = r"""        <Col xs={24} lg={10}>
        ...
        <Col xs={24} lg={14}>"""
content = re.sub(form_pattern, new_layout, content, flags=re.DOTALL)

# 5. Remove right column summary
summary_pattern = r"                \{startTime && endTime && \(\n                  <Alert.*?/>\n                \)\}\n"
content = re.sub(summary_pattern, "", content, flags=re.DOTALL)
