...
// 1. Add Collapse to imports
file = file.replace(/Checkbox, Modal \} from 'antd'/, "Checkbox, Modal, Collapse } from 'antd'");

// 2. Remove states confirmModalVisible, pendingSubmitValues, agreedInModal
...
// 3. Rewrite submitBooking and doSubmitBooking
const submitBookingStart = file.indexOf('const submitBooking = (values: SubmitFormValues) => {');
const doSubmitBookingEnd = file.indexOf('const handleBookingSuccess =', submitBookingStart);

if (submitBookingStart === -1 || doSubmitBookingEnd === -1) {
    console.error("Could not find submitBooking logic");
    process.exit(1);
}
...
const newSubmitLogic = `  const submitBooking = (values: SubmitFormValues) => {
    ...
    createMutation.mutate({
      ...
      onError: (err: any) => {
        ...
        message.error(\`Lỗi khi tạo yêu cầu đặt phòng: \${backendError}\`);
      }
    });
  }
...
file = file.substring(0, submitBookingStart) + newSubmitLogic + file.substring(doSubmitBookingEnd);

// 4. Form Layout replacement
const startToken = '<Col xs={24} lg={8}>';
const endToken = '          </Card>\n        </Col>';

const startIndex = file.indexOf(startToken);
const endIndex = file.indexOf(endToken, startIndex) + endToken.length;

if (startIndex === -1 || endIndex === -1) {
    console.error("Could not find layout replacement tokens");
    process.exit(1);
}

const newLayout = `<Col xs={24} lg={10}>
                            ...
                            <div style={{ marginBottom: 16, padding: 12, background: '#f8fafc', borderRadius: 8, border: '1px solid #e2e8f0' }}>
                              <div style={{ display: 'flex', gap: 12, alignItems: 'center', marginBottom: 12 }}>
                                {getRoomImageUrl(selectedRoom) ? (
                                  <img src={getRoomImageUrl(selectedRoom)} alt={selectedRoom.name} style={{ width: 48, height: 48, objectFit: 'cover', borderRadius: 6 }} />
                                ) : (
                ...
                <div style={{ padding: '16px', background: '#f8fafc', borderTop: '1px solid #e2e8f0', borderBottomLeftRadius: 12, borderBottomRightRadius: 12 }}>
                  ...
                  {checkLimitQuery.error && (
                    <Alert
                      ...
                      description={<div style={{ fontSize: 12 }}>{checkLimitQuery.error.message}</div>}
                    />
                  )}
...
file = file.substring(0, startIndex) + newLayout + file.substring(endIndex);

// 5. Update right column
file = file.replace(/<Col xs=\{24\} lg=\{16\}>/, "<Col xs={24} lg={14}>");

// 6. Remove old summary on right column
const summaryStart = file.indexOf('{startTime && endTime && (\n                  <Alert \n                    type="info"');
if (summaryStart !== -1) {
    const summaryEnd = file.indexOf(')}', file.indexOf('</Alert>', summaryStart));
    if (summaryEnd !== -1) {
        file = file.substring(0, summaryStart) + file.substring(summaryEnd + 2);
    }
}
